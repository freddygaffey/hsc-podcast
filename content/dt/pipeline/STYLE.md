...
- **Year 12 HSC Physics student.**
- **Already comfortable with the science and the maths.** They can rearrange an equation, handle the algebra, and they find physics genuinely interesting. They've grasped the core concepts.
- **Weak spot is the HSC, not the physics.** They don't know the *syllabus framing*, the official terminology, the way questions are marked, or the kind of examples and worked answers markers want to see.

What this means for the writing:

- **Do not re-teach the basics.** No "let's revise how to rearrange an equation", no "remember, speed is distance over time". They know. Teaching them this is condescending and wastes their time.
...
- **Use real, precise physics language.** Name the actual quantities, units, laws, and the correct technical term for things — "centripetal acceleration", "the work function", "back-EMF", "the de Broglie wavelength". The listener is doing serious physics; correct terminology respects that and is exactly what earns marks. Don't water concepts down into vague paraphrases when the proper word exists. (You still *narrate* equations rather than display blocks — see §7 — but the vocabulary should be the real thing.)

If a sentence would be obvious to any capable physics student, cut it. If a sentence helps them get a better mark in the HSC, keep it.
...
> QUESTION: Before I answer — what's the difference between the work function and the threshold frequency? Pause now, work it out, then play.
>
> NARRATOR: Right. The work function is...
...
Assume intelligence, not knowledge. The listener is smart and capable — they just haven't seen *this syllabus content* framed the HSC way before.
...
- **Link freely — including across modules.** When the home topic genuinely touches another dot-point or even another module, reference it. This is essential, not optional. Example: while teaching **projectile motion** (Advanced Mechanics), you *should* say "remember, the vertical part of this is just the constant-acceleration motion you met in Kinematics" — pulling in a concept that's properly taught in **Kinematics**. That link makes both stick. A reference is a sentence or a quick example, not a detour.
- **Forward-reference with a prelude.** You can point at something not taught yet, as long as you give a one-line prelude so the listener isn't lost ("there's a thing called the photoelectric effect — we cover it properly later — for now it just means light can knock electrons out of a metal").
...
- **Never merge two modules into one lesson.** Advanced Mechanics and The Nature of Light are separate syllabus things. You reference one *from* the other — you do **not** make a lesson that's half mechanics teaching and half light teaching. The home topic, and the module it belongs to, is always exactly one.
...
3. **The core idea.** Explain the concept in plain terms. Connect it to something they already understand where it helps ("you already know this from everyday life — the HSC just calls it X").
4. **The HSC framing.** The official terminology and how the syllabus/exam words it. This is the part they actually don't have.
5. **Worked example(s).** Concrete and exam-shaped. Go deep — several examples, with the strong-answer phrasing and full working spelled out. This is where most of the 30–45 minutes should go.
...
- For each, give the **one or two ideas that actually matter** plus the **exact syllabus term**. Not a full re-teach — a sharp reminder. "Two episodes back we did free-body diagrams — remember, every force gets its own labelled arrow, and the marker wants the forces named."
...
- **Link to at least two other syllabus topics in every episode.** When you explain something, actively reach for where else it shows up. "We're talking about circular motion here — note this is the same idea behind orbital motion, and it's exactly what the charged particle in a magnetic field relies on."
...
- **Cross-reference both directions.** Point *back* ("remember from the projectile motion episode...") and *forward* ("you'll see this again when we get to electromagnetism..."). Forward references prime the later lesson; backward ones reinforce the earlier one.
...
- **Make an acronym or acrostic** from the first letters and say it out loud as a thing to remember. "The electromagnetic spectrum in order of increasing frequency is Radio, Microwave, Infrared, Visible, Ultraviolet, X-ray, Gamma — remember it as *Raging Martians Invaded Venus Using X-ray Guns*." Then use the hook again later in the episode and in future recaps.
...
- **Tell them to write it down at the start of the exam.** "When you sit down, the first thing you dump on the page is the spectrum order." A hook they can offload onto paper in second one is worth real marks.
...
- **Pre-questions / objectives up front.** Right after the hook, pose the questions the episode answers: "By the end you'll be able to say exactly why a heavier projectile and a lighter one hit the ground at the same time." Hearing the question *first* primes attention and measurably improves what's retained.
- **Show a weak answer next to the strong one.** Don't only model the mark-earning answer — contrast it with the band-4 version. "A weak answer says *gravity pulls it down*. A strong answer says... — hear the difference?" Contrasting cases teach the marking boundary faster than a good example alone.
- **Teach through a story / real case.** Stories are remembered far better than abstract facts. Anchor a concept in a real event (a famous experiment for a discovery dot-point, a real disaster for resonance). The student recalls the story in the exam and reconstructs the concept from it.
...
- **Use the `QUESTION:` voice, and make them retrieve.** Pose the question, tell them to pause the player and actually attempt an answer (§3), *then* the narrator gives a model answer with the mark-earning phrasing and full working.
...
Some episodes are **pure case studies**: one real story, told to be genuinely gripping. Think a Veritasium video on LIGO detecting gravitational waves, or the Tacoma Narrows bridge tearing itself apart in the wind — tension, stakes, a "how did they pull this off" or "how did this go so wrong" arc. The goal is engagement and memory, not coverage.

These break the normal rules deliberately:

- **One case study per episode. One story, told well.** Not a list of three.
- **A full narrative documentary — roughly 25–35 minutes.** Give the story room: scene-setting, the people involved, the timeline beat by beat, the physics "how" woven into the drama, the tension and the payoff. Aim for the depth and craft of a Veritasium documentary. Don't pad with filler, but don't cut the story short either — earn the length with real detail and momentum, not repetition.
...
- **The syllabus tie-in happens later, in a separate episode.** A normal teaching episode down the line refers back to the case study ("remember the Tacoma Narrows story? that's resonance, which the syllabus calls...") and does the linking there. Don't do it inside the case study itself.
...
- **Name the syllabus term explicitly.** If the dot-point says "centripetal force", say "centripetal force", not "the force that keeps it going in a circle". Markers reward the right vocabulary.
...
- **Show mark-earning answers.** Don't just explain a concept — demonstrate a sentence or two, or a line of working, that would *score*. "If you write it like this, that's the mark."
- **Flag common traps.** "Students lose marks here because they confuse X with Y." Call it out by name.
- **Tie back to the syllabus, not to general science.** Interesting tangents are fine in moderation, but the spine of the script is the HSC content.

---

## 7. Writing for the text-to-speech engine

Scripts are read aloud by a TTS engine after `strip_markdown.py` cleans them, and then **listened to at 4–5× speed** in an app that also shows the script. Write **speakable prose**. The engine reads characters literally — if it looks weird written down, it'll sound weird — and a mispronounced term at 5× is unrecoverable, because there's no time to mentally reconstruct it. (Equations and data aren't spoken; they live in the appendix — see §7.1.)

**Do:**

- Write full sentences in plain text.
- Spell out things the listener needs to hear: say "electromotive force, or EMF" the first time, then "EMF" after.
- Spell out symbols and operators in words when you mean them spoken: "less than", "equals", "multiplied by", "squared", "per second".
- **Make pronunciation bulletproof.** If a term, symbol, unit, or name could trip the engine, write it the way it should *sound* (e.g. "λ" → "lambda", "Wb" → "weber", "μ" → "mu", "Δv" → "delta v"). At high speed there's no second chance to parse a garbled word.
- Use commas and full stops to control pacing within a sentence.

**Don't (in the spoken portion):**

- No equation blocks in the spoken text. *Describe* the equation in English where you teach it — "kinetic energy max equals Planck's constant times frequency, minus the work function", not a literal symbolic line. (The full working lives in the appendix; see §7.1.)
...
- No raw symbols, unit abbreviations, or scientific-notation written numerically that you don't want read literally — say "six point six three times ten to the minus thirty-four", not `6.63e-34`.
...
Describe an equation by what it says, not by pasting the symbols into the narration. This is a *format* rule, not a vocabulary one — use the real quantities and precise terms freely (§1); just say them aloud. Talk about "acceleration equals the change in velocity over time", not a literal `a = Δv / t`.

### 7.1 The appendix: equations, derivations & data (the reading companion)
...
- **Equations, derivations, worked solutions, and data tables are supplementary, and live in an appendix at the very bottom of the file** — after an `## Appendix` heading. The TTS pipeline stops there: nothing in the appendix is ever spoken. (No diagrams — see `SUPPLEMENTARY.md`.)

How to use it:

- **Put the full equation, derivation, or worked solution in the appendix as a labelled listing.** Use a plain ` ```text ` fence with one step per line so the working is clear. Label each one ("Listing 1", "Listing 2").
- **Show the full working for examinable calculations.** When a topic could be examined as a calculation, give the complete worked solution in the appendix — substitution, rearrangement, units, final answer — so the student rehearses the exact method they'll write for marks. The working is itself a teaching point. (Format details in `SUPPLEMENTARY.md` §3.)
- **Reference it from the narration, but never depend on it.** "I've put the full working in Listing 1 if you want to read it later" is good. The spoken explanation must still stand alone — *don't* say "as you can see here" and trust they're looking.
- **Keep labels stable** so recaps and reviews can point back to "Listing 1 from the photoelectric episode".
...
| `M1` | Kinematics | 11 |
| `M2` | Dynamics | 11 |
| `M3` | Waves and Thermodynamics | 11 |
| `M4` | Electricity and Magnetism | 11 |
| `M5` | Advanced Mechanics | 12 |
| `M6` | Electromagnetism | 12 |
| `M7` | The Nature of Light | 12 |
| `M8` | From the Universe to the Atom | 12 |

- **Module-review episodes (§5.3)** use a high `LL` so they sort to the end of their block,
  plus a clear marker: `M7-99-Module-Review-The-Nature-Of-Light`.
- **Case-study episodes (§5.7)** are the one exception to the naming scheme: name them
  `case_descriptive_title` in lower snake_case — e.g. `case_ligo_detects_gravitational_waves`,
  `case_the_tacoma_narrows_collapse`. They sit outside the teaching sequence (no module, no `LL`)
  and get tied in later; the `case_` prefix groups them together. Keep the title evocative, not
  syllabus-y.
- Keep all filenames in `Title-Case-With-Hyphens` — no spaces, no underscores (except the
  `case_` prefix).
...
- [ ] Assumes a capable physics student — no basics re-taught.
- [ ] Uses the exact syllabus terminology.
- [ ] Contains at least one exam-shaped worked example with mark-earning phrasing and full working.
...
- [ ] Spoken narration is self-contained — learns fine audio-only; equations/derivations/data are in the `## Appendix`, referenced by stable labels but never depended on (§7.1).
- [ ] Reads cleanly aloud — no equation blocks or markdown in the spoken part — and tricky terms, symbols, and units are spelled for correct pronunciation.

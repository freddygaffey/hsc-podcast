...
$SSH "$SERVER" "cd $SRVDIR && pgrep -f '[p]ython3 _downloader.py' >/dev/null \
...
$SSH "$SERVER" "cd $SRVDIR && pgrep -f '[p]ython3 _nesa.py' >/dev/null \
  && echo '   already running' \
...
$SSH "$SERVER" "cd $SRVDIR && grep -q '=== acehsc done' _acehsc.log 2>/dev/null && echo '   already complete' \
  || { pgrep -f '[p]ython3 _acehsc.py' >/dev/null && echo '   already running' \
